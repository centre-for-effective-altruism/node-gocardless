...
const ALLOWED_REQUEST_METHODS = ['GET', 'POST', 'PUT', 'PATCH', 'DELETE']
...
GoCardless.prototype.request = function ({method, resource, data, query, options}) {
  ...
  if (!ALLOWED_REQUEST_METHODS.includes(method.toUpperCase())) throw new RangeError(`Invalid request method ${method}. Must be one of [${ALLOWED_REQUEST_METHODS.join(`, `)}].`)
  if (!method) throw new RangeError(`No method supplied`)
  // Add auth headers
